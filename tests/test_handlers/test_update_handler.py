...
async def test_update_user(client, create_user_in_database, get_user_from_database):
    ...
    user_data_updated = {
        "name": "Test-hard",
        "surname": "Testovich-hard",
        "email": "Test-hard@test.com",
    }
    ...
    assert user_from_db["is_activ"] is user_data["is_active"]
...
@pytest.mark.parametrize("user_data_updated, expected_status_code, expected_detail", [
    (
            {},
            422,
            {"detail": "At least one parameter for user update info shoukd be provided"}
    ),
    (
            {"name": "123"},
            422,
