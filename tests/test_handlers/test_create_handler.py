...
async def test_create_user(client, get_user_from_database):
    ...
    assert user_from_db["is_activ"] is True
...
async def test_create_user_duplicate_error(client, get_user_from_database):
    ...
    assert data_from_resp["surname"] == user_data["surname"]
    assert data_from_resp["email"] == user_data["email"]
    assert data_from_resp["is_activ"] is True
    user_from_db = await get_user_from_database(data_from_resp["user_id"])
    assert len(user_from_db) == 1
    user_from_db = dict(user_from_db[0])
    assert user_from_db["name"] == user_data["name"]
    ...
    assert 'duplicate key value violates unique constraint "user_email_kye"' in resp.json()["detail"]
